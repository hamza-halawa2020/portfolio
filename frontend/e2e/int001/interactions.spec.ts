...
test.beforeEach(async ({ page }) => {
  page.on('console', (message) => {
    ...
    expect(text).not.toMatch(/hydration|visitor_id_hash|ip_hash|user_agent_hash|secret|admin_notes/i);
...
test('records project views once after browser rendering and preserves idempotency on reload', async ({ page }, testInfo) => {
...
test('does not block project content when browser view tracking fails', async ({ page }, testInfo) => {
  const slug = projectSlug('e2e-project-view-failure', testInfo);
  await page.route(`**/api/v1/projects/${slug}/views`, async (route) => {
    await route.fulfill({ contentType: 'application/json', status: 500, body: JSON.stringify({ message: 'Expected E2E view failure' }) });
  });

  await page.goto(`/en/projects/${slug}`);
  await expect(page.getByRole('heading', { level: 1, name: 'E2E view failure project' })).toBeVisible();
  await expect(page.getByText('Body for E2E view failure project.')).toBeVisible();
});

test('loads like state from the visitor cookie and keeps it after reload', async ({ page, context }, testInfo) => {
...
test('prevents rapid duplicate like submissions and preserves state on failure', async ({ page }, testInfo) => {
  ...
  await page.route(`**/api/v1/projects/${failureSlug}/likes`, async (route) => {
    if (route.request().method() === 'POST') {
      await route.fulfill({ contentType: 'application/json', status: 500, body: JSON.stringify({ message: 'Expected E2E like failure' }) });
    } else {
...
test('submits contact messages with validation, consent, live confirmation, and no browser storage leakage', async ({ page }) => {
  ...
  await expect(page.locator('app-contact-form').getByLabel('Name')).toHaveAttribute('aria-invalid', 'true');
  ...
  expect(await page.locator('body').innerText()).not.toMatch(/visitor_id_hash|ip_hash|admin_notes/i);
});
...
test('submits testimonials for moderation without immediate public exposure', async ({ page }) => {
  ...
  await expect(page.locator('app-testimonial-form').getByText('Please check this field.').first()).toBeVisible();
  ...
  await expect(page.locator('body')).not.toContainText(/contact_email|reviewed_at|admin_notes|internal_notes/i);
});

test('renders configured WhatsApp action and hides it when configuration is missing on client navigation', async ({ page }) => {
  await page.goto('/en/contact');
  ...
  expect(whatsappHref.searchParams.get('text')).toBe('Hello, I would like to discuss an E2E project.');

  await page.route('**/api/v1/site**', async (route) => {
    await route.fulfill({
      contentType: 'application/json',
      body: JSON.stringify({ data: { navigation: [], services: [], settings: {}, skills: [], social_links: [] } }),
    });
  });
  await page.goto('/en');
...
test('supports Arabic RTL interaction pages without private data or failed browser requests', async ({ page }) => {
...
async function fillContactForm(page: Page, values: { email: string; message: string; name: string }): Promise<void> {
...
async function fillTestimonialForm(page: Page, values: { content: string; email: string; name: string }): Promise<void> {
